...
// Hou comment: consider using a more descriptive variable name such as "openModalButton". Abbreviating a word sometimes makes a variable difficult to read/understand.
...
// Hou comment: instead of using JavaScript, we can add the styles from lines 23-36 in the styles.css file.
//Styling Attributes
...
//Music Category
// Hou comment: I'd move all the selectors to the top of the file for easier discoverability
...
/** 
	Hou comment: Refactoring suggestion for lines 410-462

	function createChoiceButton(id, text) {
		let choiceButton = document.createElement('BUTTON');
		choiceButton.setAttribute('id', button);
		choiceButton.innerHTML = text;
		choiceButton.classList.add('choice-button-style');
		choiceButton.addEventListener('click', checkMusicAnswer);
		choiceButton.addEventListener('mouseover', (event) => {
	...
	function displayMusicQuestion() {
  		...
  		questionDisplay.innerHTML = trivia.question;
			...
			const choiceButtons = choiceLetters.map(choiceLetter => {
				let newChoiceButton = createChoiceButton(choiceLetter, trivia[`choice${choiceLetter}`])
				questionDisplay.appendChild(newChoiceButton);
			})
	}
*/
function displayMusicQuestion() {
  ...
  // Hou comment: how about using choiceABtn.classList.add('choice-button-style') to add styling?
...
// Hou comment: how would you refactor the code below based on the suggestion on line 377 above?
